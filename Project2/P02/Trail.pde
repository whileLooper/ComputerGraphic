...
/**************************** HEADER ****************************
 ...
 Student: Bo Chen
 Project number: P02
 Project title: Parabola
 Date of submission: 8/31/2014
 This project's idea I have been collaborated with Yang, Shen.
*****************************************************************/
...
pt A,B,C, tempP;    //centroids point 
//pts parabola = new pts();  //parabola points set
int counter = 0;          // a counter to count 30 points
float time = 1/30f;        //set time as 1 frame
pts parabolaLine;  //a point set
...
void drawParabola(float x, float y){
   ...
   if(counter == 0) {
     ...
     C = new pt(tempX/10, tempY/10);
     
     counter++;  
   }
   else if(counter < 30 && (trailX[counter - 1] != x || trailY[counter - 1] != y)){
     trailX[counter] = x; 
     trailY[counter] = y;
     counter++; 
   }
...
void show(){    
   ...
   if (counter >= 30){
      ...
      vec AB = V(A, B);  //setting vector AB
      vec BC = V(B, C);  //setting vector BC
      vec AC = V(A, C);  //setting vector AC

      vec accG = new vec(BC.x-AB.x, BC.y-AB.y); //calculating accelaration 
      arrow(B, accG, "G");  //showing C point accelaration
      
      vec V = new vec(AC.x/2, AC.y/2);  //calculating velocity
      arrow(B, V, "V");
      parabolaLine = new pts();
      parabolaLine.declare();        //init the point set
      
      //adding 50 predict points into point set.
      for (int i = 0; i < 100; i++){
        float newTime = time * (i + 1);
        tempP = new pt(B.x + newTime*V.x + 0.5*(newTime*newTime)*accG.x, B.y + newTime*V.y + 0.5*(newTime*newTime)*accG.y);
        parabolaLine.addPt(tempP);
      }
      
      parabolaLine.drawCurve();  //drawing the parabola using point set
    }
