...
float t=0.5, f=0, s=0;
boolean animate=true, showStrokes=true, showPoints = true, trigger = false;
pt Lst, Lt, Ls;
...
void draw() {      // executed at each frame
  ...
  if (animate){
   ...
   for(float s = 0; s < 1.01; s += 0.01) leftCurve(P.G, s, 0).v();
   endShape(); 
   
   strokeWeight(3); 
   stroke(green); beginShape();
   for(float s = 0; s < 1.01; s += 0.01) rightCurve(P.G, s, 0).v();
   endShape(); 
  
   strokeWeight(3); 
   stroke(black); beginShape();
   for(float t = 0; t < 1.01; t += 0.01) topCurve(P.G, 0, t).v();
   endShape(); 
   
   strokeWeight(3); 
   stroke(blue); beginShape();
   for(float t = 0; t < 1.01; t += 0.01) downCurve(P.G, 0, t).v();
   endShape(); 
  ...
  if(key == 'a'){
    if(trigger){
      if(t > 1) { trigger = false;} else t += 0.01;
    }
    else if(!trigger){
      if(t < 0) { trigger = true;} else t -= 0.01;
...
name ="Student: Bo Chen", 
menu="?:'a' = amiationMode, 's' = mouseMode, Q:quit", 
