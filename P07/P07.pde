...
void draw() {
  ...
  pushMatrix();   // to ensure that we can restore the standard view before writing on the canvas
    ...
    if(showControlPolygon) {
      fill(green); P.drawBalls(4); // draw curve P as cones with ball ends
      //fill(red); drawBorders(Q.G, (1/30f)); Q.drawBalls(4); // draw curve Q
    ...
    fill(green);
    newP = drawBorders(P.G, 0.01f);
    fill(red);
    newQ = drawBorders(Q.G, 0.01f);
    fill(cyan); 
    if(animating) {
      
    
     PtoQ =  PtQ.setToL(P,s,Q); 
    // PtQ.drawClosedLoop(); 
     shadeSurface(PtoQ.G, 0.01f);
     // drawBorders(P.G, (1/30f));
     // drawBorders(Q.G, (1/30f));....
...
String title ="2014: Coons patch editor & animator in 3D", name ="Bo Chen",
