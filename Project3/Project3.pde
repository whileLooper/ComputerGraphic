...
// Modified by student:  Bo Chen
//Collaboration statement: Worked with Shen Yang on how it should be designed and the Math behind it.
...
void draw() {      // executed at each frame
  ...
  initial = P.G;
   if(showTriangles) {
    ...
    fill(black); 
    label(R.G[0],"A"); label(R.G[1],"B"); label(R.G[2],"C"); label(R.G[3], "D");
    cp.label("Centroid");
    //label(A1,"A"); label(B1,"B"); label(C1,"C"); label(D1, "D");
    }
  ...
  if (animate) {
    ...
    time+=1/100000f;
    System.out.println("P.G[0]: " + P.G[0].x);
    }
  }
...
void mouseDragged() {
  ...
  if (!animate) {
  if (!keyPressed || (key==' ')){
      //Q.dragPicked();   // drag selected point with mouse
  ...
  change=true;
  //System.out.print("centroid: (" + P.Centroid().x + ", " + P.Centroid().y + ")" );
  }  

//**************************** text for name, title and help  ****************************
String title ="Steady pattern of strokes", 
       name ="Student: Bo Chen 9/8/2014",
       
       menu="?:(show/hide) help, !:snap picture, ~:(start/stop) recording frames for movie, Q:quit",
       guide="drag to edit triangle vertices, 'a' is for the animation movement"; // help info
